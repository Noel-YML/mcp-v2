...
- [`docs/CAPABILITY_SURFACE_AND_PARAMETER_CONTRACT.md`](CAPABILITY_SURFACE_AND_PARAMETER_CONTRACT.md) — the frozen bounded capability surface: exact parameter contracts and enums per capability, declared pair and invalid-combination matrices, parameter ownership (model-selectable vs server-injected), existing action registries vs proposed interaction intents, governed output extensions, implementation order, and canonical-fixture requirements
- [`docs/S01_TEMPLATE1_TARGET_COVERAGE.md`](S01_TEMPLATE1_TARGET_COVERAGE.md) — the S01 gate: agreed initial Template 1 (Performance) question coverage (families A + D₁ on one capability), explicitly out-of-scope analytical operations, Noel/Christine ownership split, and acceptance criteria
- [`docs/N06_CALCULATION_OWNERSHIP_AND_PROVENANCE.md`](N06_CALCULATION_OWNERSHIP_AND_PROVENANCE.md) — the N06 closure record: the four-class ownership model, the current field ownership table, the allowed rendering-only operations, the provenance chain, the structural enforcement points and the test enforcing each rule, and the rule that any new analytical value must name its deterministic producer first

**Governed result contract (N05).** `mcp/governed_result/` implements the common governed envelope plus its first typed payload (`payload_type = "metric_set"`), with a pure non-computing projection from the existing trusted `RevenueDigestResult`. It is additive: no tool's wire contract consumes it yet, and `RevenueDigestResult`, `AnalyticsResult` and `StoredResult` are unchanged. See [`docs/GOVERNED_RESULT_ARCHITECTURE.md`](GOVERNED_RESULT_ARCHITECTURE.md) section 6.4.

**Governed result wire contract (N07).** The JSON wire format is project-owned and frozen for the one implemented payload: the canonical JSON Schema (Draft 2020-12) lives at `mcp/governed_result/wire/governed_result_envelope.schema.json`, canonical and negative fixtures at `mcp/governed_result/wire/fixtures/`, and a standard-library semantic validator at `mcp/governed_result/validation.py`. The schema and fixtures are the cross-language contract for future C#/TypeScript consumers; `jsonschema` is a test-only dependency. Not wired into any tool — UI binding (N11) must validate before rendering. See section 6.5 of the same document.
