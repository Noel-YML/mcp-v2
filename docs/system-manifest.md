...
## 12. H1/H1.1 — Azure App Service hosting for webchat (staging, in progress, Aug 2026)

**Isolation decision:** webchat's future App Service deployment will be a brand-new, isolated stack — resource group `rg-ask-ariel-web-v2`, plan `asp-ask-ariel-web-v2`, web app `ask-ariel-web-v2` (+ a staging slot), Key Vault `kv-ariel-web-v2-<suffix>`, Application Insights `appi-ask-ariel-web-v2`, region East US. This repository's `Analytics` resource group already contains two unrelated, pre-existing, closely-named stacks — a "prod"-tagged Node.js App Service (`ask-ariel-ymlinnovation-prod-*`) and a parallel Container Apps stack (`ca-ask-ariel-*-dev`, `kv-ariel-h0-dev-*`) — neither is Python/Flask, and **no future provisioning step may target, modify, or reuse either one's Key Vault, identities, storage, or secrets.**
...
**Runtime:** Python 3.14 (`az webapp list-runtimes --os linux` confirms `PYTHON:3.14` is available; verified locally compatible with gunicorn 26.2.0, Flask, azure-ai-projects, azure-identity, openai, and mcp). `gunicorn>=23.0.0,<27.0.0` added to `webchat/requirements.txt`.

**Startup command** (verified against Microsoft's own documented Linux Python container behavior — App Service's built-in Flask auto-detection only looks for `app.py`/`application.py`, and `server.py` is neither, so a custom startup command is mandatory regardless; the container's default extra arguments are `--bind=0.0.0.0 --timeout 600`, with **no** `$PORT` in Microsoft's own documented Python examples — Python differs here from some other language stacks):

```bash
gunicorn --bind=0.0.0.0 --timeout 600 --workers=1 --threads=1 --chdir webchat --access-logfile - --error-logfile - server:app
```

`--chdir webchat` both `os.chdir()`s and inserts `webchat/` onto `sys.path` (confirmed by reading the installed gunicorn's `Application.chdir()`), which is what lets `server.py`'s top-level `import mcp_app` / `agent_contract` / `presentation_validator` / `actions_registry` resolve exactly as they do when run locally via `python server.py` from inside `webchat/`.
